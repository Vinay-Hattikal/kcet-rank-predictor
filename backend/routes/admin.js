...
router.post('/colleges/upload', protectAdmin, upload.array('files'), (req, res) => {
  console.log('\n=== NEW COLLEGE UPLOAD REQUEST RECEIVED (BACKGROUND) ===');
  ...
  res.status(202).json({ 
    success: true, 
    message: 'Upload received! Processing in the background. Colleges will be live soon.' 
  });
  ...
  (async () => {
    try {
                ...
                if (!headersMapped) {
                  ...
                  if (name && name.length > 3 && !name.toLowerCase().includes('college name') && !name.toLowerCase().includes('institute name')) {
                    ...
                    await upsertCollegeSafe(normalizedName, {
                      ...
                      fees: { government: govtFee, management: mgmtFee }
                    });
                    totalUpdated++;
                  }
                }
              }
            } catch (err) { console.error('Background Row error (Colleges):', err); }
            stream.resume();
          });
      ...
      console.log(`[BACKGROUND COMPLETE] Updated ${totalUpdated} college records.`);
    } catch (bgError) {
      console.error('[BACKGROUND CRASH (Colleges)]', bgError);
    }
  })();
});
...
router.post('/cutoffs/upload', protectAdmin, upload.array('files'), (req, res) => {
  console.log('\n=== NEW CUTOFF UPLOAD REQUEST RECEIVED (BACKGROUND) ===');
  if (!req.files || req.files.length === 0) {
  ...
  res.status(202).json({ 
    success: true, 
    message: 'Upload received! Processing in the background. Data will be live in 1-2 minutes.' 
  });
  ...
  (async () => {
    try {
          ...
          stream.on('data', async (row) => {
            ...
            try {
              ...
              if (isCOMEDKHeader) {
                ...
                if (firstCol && firstCol.length > 3 && !firstCol.toLowerCase().includes('course') && !firstCol.toLowerCase().includes('college:')) {
            ...
            } catch (err) { console.error('Background Row error (Cutoffs):', err); }
            stream.resume();
          });
      ...
      console.log(`[BACKGROUND COMPLETE] Parsed ${totalCutoffsParsed} cutoff records.`);
    } catch (bgError) {
      console.error('[BACKGROUND CRASH (Cutoffs)]', bgError);
    }
  })();
});
